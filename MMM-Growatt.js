Module.register("MMM-Growatt", {
  ...
  createTable: function () {
    ...
    this.config.rowToDisplay.forEach((row) => {
      console.log(row);
      const staticData = rowMappings[row] || {
        label: "Unknown",
        image: "state_spf5000.png",
      };
    ...
    footerRow.innerHTML = `
            <td colspan="4" id="footer">Static taken at: ${
              this.growattData[0]?.staticTakenAt || "--"
            }</td>
        `;
  ...
  updateTable: function () {
    ...
    this.config.rowToDisplay.forEach((row) => {
      ...
      if (row === "solar") {
        let solarValue =
          parseInt(this.growattData[0].ppv1 || 0) +
          parseInt(this.growattData[0].ppv2 || 0);
        ...
        valueElement.innerHTML = `SoC: ${
          this.growattData[0].stateOfCharge || "0"
        }%<br>${chargeDischarge}: ${chargeDischargeValue}W`;
      } else if (row === "loadPower") {
        valueElement.innerHTML = `${
          this.growattData[0].consumptionPower || "0"
        }W/${this.growattData[0].rateVA || "--"}VA`;
      } else {
    ...
    if (footerElement) {
      footerElement.innerHTML = `Static taken at: ${
        this.growattData[0].staticTakenAt || "--"
      }`;
    }
    return wrapper;
  ...
  generateSolarLine: function () {
    ...
    var ppvData = this.growattData[0].ppv
      ? this.growattData[0].ppv
      : this.growattData[0].ppv1
      ? parseInt(this.growattData[0].ppv1) + parseInt(this.growattData[0].ppv2)
      : 0;
  ...
  generateGridLine: function () {
    ...
    if (this.growattData[0].gridPower > 0) gridLine.classList.add("gridactive");
    ...
    if (this.growattData[0].gridPower) {
      ...
      if (this.growattData[0].growattType === "mix") {
        gridLabel.innerHTML = `${this.growattData[0].exportToGrid}KW`;
      } else {
  ...
  generateBatteryLine: function () {
    ...
    if (this.growattData[0].charging < 0 || this.growattData[0].charging > 0) {
      ...
      (`${this.growattData[0].stateOfCharge}` > 35) &
      (`${this.growattData[0].discharging}` > 0)
    ) {
